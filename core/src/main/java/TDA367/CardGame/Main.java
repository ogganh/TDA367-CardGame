...
public class Main extends ApplicationAdapter {
    ...
    private void input() {
        ...
        if (Gdx.input.isKeyPressed(Input.Keys.NUM_1)) {
            ...
            Gdx.app.log("Action",
                    "Player " + Integer.toString(gameContext.getCurrentPlayerIndex() + 1) + " asked for ACEs");
        }
        if (Gdx.input.isKeyPressed(Input.Keys.NUM_2)) {
            gameContext.handleTurn(new PlayerAction(gameContext.getCurrentPlayerIndex(), "ask", "TWO", "HEARTS"));
            Gdx.app.log("Action",
                    "Player " + Integer.toString(gameContext.getCurrentPlayerIndex() + 1) + " asked for TWOs");
        }
        if (Gdx.input.isKeyPressed(Input.Keys.NUM_3)) {
            gameContext.handleTurn(new PlayerAction(gameContext.getCurrentPlayerIndex(), "ask", "THREE", "HEARTS"));
            Gdx.app.log("Action",
                    "Player " + Integer.toString(gameContext.getCurrentPlayerIndex() + 111) + " asked for THREEs");
        }
